...
>
> 当前规模：**70 篇文章**（脉络一 66 + 脉络二 2 + 脉络三 2）+ **1 项已跟踪产品**（不计入文章数）。最近一次同步：2026-08-01。
...
> 本段不参与 `### N. ...` 的全局编号，不计入 70 篇文章总数；与上方编号正文严格区分，避免污染脉络计数。
...
> 这里收录的是**开源产品 / 框架 / 工具**，不是文章。本段不参与"### N. ..." 的全局编号，不计入 70 篇的文章总数。
...
> 2026-05 起各轮调研中已甄别、但**暂不值得做成正式文章**的产品页 / README / 短 bliki / 发布稿 / 工程随笔。本段不参与 `### N.` 编号，不计入 70 篇文章总数。
